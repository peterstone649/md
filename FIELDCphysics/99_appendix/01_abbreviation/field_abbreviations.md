# Physics Field Abbreviations (ABBREV_FOR_FIELDCPHYSICS) **[PRIO: LOW]**

**This document contains abbreviations and acronyms specific to the physics domain within the FIELDCphysics framework.**

## Physics Abbreviations

### **Fundamental Constants**
- **c**: Speed of light in vacuum (299,792,458 m/s)
- **G**: Gravitational constant (6.67430 × 10⁻¹¹ m³ kg⁻¹ s⁻²)
- **h**: Planck constant (6.62607015 × 10⁻³⁴ J·s)
- **ℏ**: Reduced Planck constant (h/2π)
- **e**: Elementary charge (1.602176634 × 10⁻¹⁹ C)
- **k_B**: Boltzmann constant (1.380649 × 10⁻²³ J/K)
- **ε₀**: Vacuum permittivity (8.8541878128 × 10⁻¹² F/m)
- **μ₀**: Vacuum permeability (1.25663706212 × 10⁻⁶ H/m)

### **Physical Quantities**
- **F**: Force (Newton, N)
- **E**: Energy (Joule, J)
- **p**: Momentum (kg·m/s)
- **L**: Angular momentum (kg·m²/s)
- **P**: Power (Watt, W)
- **W**: Work (Joule, J)
- **Q**: Heat (Joule, J)
- **T**: Temperature (Kelvin, K)
- **S**: Entropy (J/K)
- **U**: Internal energy (Joule, J)
- **H**: Enthalpy (Joule, J)
- **A**: Helmholtz free energy (Joule, J)
- **G**: Gibbs free energy (Joule, J)

### **Mathematical Operators**
- **∇**: Nabla operator (gradient)
- **Δ**: Laplace operator
- **∂**: Partial derivative
- **∫**: Integral
- **∑**: Summation
- **∏**: Product
- **∀**: For all (universal quantifier)
- **∃**: There exists (existential quantifier)
- **→**: Vector quantity or limit
- **↔**: If and only if (logical equivalence)

### **Physics Theories and Models**
- **CM**: Classical Mechanics
- **QM**: Quantum Mechanics
- **QFT**: Quantum Field Theory
- **QED**: Quantum Electrodynamics
- **QCD**: Quantum Chromodynamics
- **GR**: General Relativity
- **SR**: Special Relativity
- **SM**: Standard Model
- **EM**: Electromagnetism
- **Thermo**: Thermodynamics
- **Stat Mech**: Statistical Mechanics
- **CM**: Continuum Mechanics
- **Fluid Dyn**: Fluid Dynamics

### **Experimental Techniques**
- **AFM**: Atomic Force Microscopy
- **STM**: Scanning Tunneling Microscopy
- **SEM**: Scanning Electron Microscopy
- **TEM**: Transmission Electron Microscopy
- **XRD**: X-ray Diffraction
- **XPS**: X-ray Photoelectron Spectroscopy
- **NMR**: Nuclear Magnetic Resonance
- **EPR**: Electron Paramagnetic Resonance
- **FTIR**: Fourier Transform Infrared Spectroscopy
- **Raman**: Raman Spectroscopy
- **SIMS**: Secondary Ion Mass Spectrometry
- **ToF-SIMS**: Time-of-Flight SIMS

### **Particle Physics**
- **LEP**: Large Electron-Positron Collider
- **LHC**: Large Hadron Collider
- **CMS**: Compact Muon Solenoid
- **ATLAS**: A Toroidal LHC Apparatus
- **ALICE**: A Large Ion Collider Experiment
- **LIGO**: Laser Interferometer Gravitational-Wave Observatory
- **Fermilab**: Fermi National Accelerator Laboratory
- **CERN**: European Organization for Nuclear Research
- **SLAC**: Stanford Linear Accelerator Center
- **DESY**: Deutsches Elektronen-Synchrotron

### **Astrophysics and Cosmology**
- **CMB**: Cosmic Microwave Background
- **WMAP**: Wilkinson Microwave Anisotropy Probe
- **Planck**: Planck Satellite
- **HST**: Hubble Space Telescope
- **JWST**: James Webb Space Telescope
- **ESA**: European Space Agency
- **NASA**: National Aeronautics and Space Administration
- **EHT**: Event Horizon Telescope
- **LIGO**: Laser Interferometer Gravitational-Wave Observatory
- **Virgo**: Virgo Interferometer

### **Units and Dimensions**
- **SI**: International System of Units
- **CGS**: Centimeter-Gram-Second system
- **MKSA**: Meter-Kilogram-Second-Ampere system
- **eV**: Electronvolt
- **MeV**: Mega-electronvolt
- **GeV**: Giga-electronvolt
- **TeV**: Tera-electronvolt
- **amu**: Atomic mass unit
- **u**: Unified atomic mass unit
- **Da**: Dalton (alternative to amu)

### **Mathematical Constants**
- **π**: Pi (3.14159...)
- **e**: Euler's number (2.71828...)
- **γ**: Euler-Mascheroni constant (0.57721...)
- **φ**: Golden ratio (1.61803...)
- **i**: Imaginary unit (√-1)
- **∞**: Infinity

### **Physics Laws and Principles**
- **N1L**: Newton's First Law (Inertia)
- **N2L**: Newton's Second Law (F = ma)
- **N3L**: Newton's Third Law (Action-Reaction)
- **1st Law**: First Law of Thermodynamics
- **2nd Law**: Second Law of Thermodynamics
- **3rd Law**: Third Law of Thermodynamics
- **0th Law**: Zeroth Law of Thermodynamics
- **COP**: Coefficient of Performance
- **Carnot**: Carnot efficiency
- **Stefan-Boltzmann**: Stefan-Boltzmann law
- **Wien**: Wien's displacement law

### **Computational Physics**
- **MD**: Molecular Dynamics
- **MC**: Monte Carlo
- **DFT**: Density Functional Theory
- **FEM**: Finite Element Method
- **FDM**: Finite Difference Method
- **CFD**: Computational Fluid Dynamics
- **PIC**: Particle-in-Cell
- **GPU**: Graphics Processing Unit
- **CPU**: Central Processing Unit
- **MPI**: Message Passing Interface
- **OpenMP**: Open Multi-Processing

### **Material Properties**
- **E**: Young's modulus
- **G**: Shear modulus
- **K**: Bulk modulus
- **ν**: Poisson's ratio
- **ρ**: Density
- **σ**: Stress
- **ε**: Strain
- **α**: Thermal expansion coefficient
- **κ**: Thermal conductivity
- **c_p**: Specific heat capacity at constant pressure
- **c_v**: Specific heat capacity at constant volume

### **Quantum Numbers**
- **n**: Principal quantum number
- **l**: Azimuthal quantum number
- **m_l**: Magnetic quantum number
- **m_s**: Spin quantum number
- **s**: Spin quantum number
- **j**: Total angular momentum quantum number
- **m_j**: Magnetic quantum number for total angular momentum

### **Field Types**
- **EM**: Electromagnetic field
- **Grav**: Gravitational field
- **Scalar**: Scalar field
- **Vector**: Vector field
- **Tensor**: Tensor field
- **Spinor**: Spinor field
- **Gauge**: Gauge field
- **Higgs**: Higgs field

### **Field Classification Levels**
- **C**: Core field (fundamental, foundational physics)
- **M**: Main field (major branch of physics)
- **S**: Specialized subfield (specific application area)

---

**Note**: This abbreviation list is maintained as part of the FIELDCphysics domain conventions and should be updated as new physics concepts and techniques are integrated into the framework.